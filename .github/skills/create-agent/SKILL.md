...
- Top-level `model:` is required — pick one, do not leave it off. Omitting it silently inherits whatever gh-aw's
  ...
  `claude-opus-5`. If a run dies with *"unsupported or unrecognized by this AWF version"*, your `gh aw` build is
  stale — `gh extension upgrade gh-aw` and recompile, do not downgrade the model.
  See [Models](../../../docs/operations.md#models).
